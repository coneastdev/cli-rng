...
void help() {
    std::cout << "show debug info\n--s / --show [next command]\nUse before a command to show the debug information\n\ncoin flip\n--c / --coin [number] [number]\nRuns a coin flip, by default it is 50-50 but you can add the probability ratio\n\ndice roll\n--d / --dice [number] [number]\nRuns a dice roll, you can specify the faces on the dice and the amount of dice, by default it is one d6";
}
